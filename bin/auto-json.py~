...
parser = argparse.ArgumentParser(description="""Makes a json file to house the data for FRB rate calculation.
sample command: python auto-json.py -N <name> -Y <year> -n <nFRBs> -S <FWHM_2> -R <radius> -b <beams> -t <tpb> -f <flux> -F <observing frequency>""",formatter_class=argparse.RawTextHelpFormatter)
...
parser.add_argument('-F', '--freq', help='observing frequency(MHz), do not seperate values with commas - only spaces are needed', action='store', nargs='+', type=float, default=[], required=False)
