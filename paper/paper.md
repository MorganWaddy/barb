...
![Corner plot displaying the one-dimensional projections of $\log(\mathcal{R})$ and $\alpha$, as well as a two-dimensional projection of the samples to show the covariances. The vertical dotted lines represent the quantiles and the contours are 1 $\sigma$, 2 $\sigma$, 3 $\sigma$, etc. \label{fig:corner}](rates_mc.png)
