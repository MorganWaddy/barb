...
![Corner plot displaying the one-dimensional projections of $\log(\mathcal{R})$ and $\alpha$, as well as a two-dimensional projection of the samples to show the covariances. The vertical dotted lines represent the quantiles that specify where a proportion of the distribution of data resides. Accordingly, a quarter of the data have a value less than the first quartile and three-quarters of the data have a value less than the third quartile. The contours are 1$\sigma$, 2$\sigma$, 3$\sigma$, etc. \label{fig:corner}](rates_mc.png)
